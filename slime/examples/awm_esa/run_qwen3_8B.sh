#!/bin/bash
#
# ESA (Execution-Status Awareness) RL training with Qwen3-8B on slime.
#
# Three-step workflow:
...
eval "$(micromamba shell hook -s bash)" && micromamba activate slime
...
RUN_LOG="${RUN_LOG:-${LOG_DIR}/run_qwen3_8B_${RUN_TS}.log}"
...
# ── Paths ────────────────────────────────────────────────────────────────────
HF_CKPT="${HF_CKPT:-${SLIME_DIR}/../models/Qwen3-8B}"

# Download model if not present
if [ ! -d "${HF_CKPT}" ]; then
    echo "Downloading Qwen3-8B..."
    huggingface-cli download Qwen/Qwen3-8B --local-dir "${HF_CKPT}"
fi

CKPT_ARGS=(
   --hf-checkpoint "${HF_CKPT}"
   --save "${SLIME_DIR}/../checkpoints/awm_esa_qwen3_8B/"
   --save-interval 3
)
